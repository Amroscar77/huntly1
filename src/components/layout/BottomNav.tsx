import { Home, PlusCircle, Bell, User, ShoppingCart } from 'lucide-react';
...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full z-50 bg-white/80 backdrop-blur-xl border-t border-surface-container shadow-[0_-4px_20px_0_rgba(0,0,0,0.05)] rounded-t-[2rem] px-4 pb-6 pt-3 flex justify-around items-center">
      ...
      <NavLink to="/shopping-list" icon={<ShoppingCart size={24} />} label="List" active={location.pathname === '/shopping-list'} />
      <NavLink to="/add" icon={<PlusCircle size={24} />} label="Add" active={location.pathname === '/add'} />
      <NavLink to="/watchlist" icon={<Bell size={24} />} label="Alerts" active={location.pathname === '/watchlist'} />
