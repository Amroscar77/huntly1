...
export const MarketService = {
  ...
  async submitPrice(submission: Omit<PriceSubmission, 'id' | 'timestamp' | 'confirmations' | 'verified' | 'geohash'>) {
    ...
    batch.set(submissionRef, {
      ...
      confirmedBy: [],
      verified: false,
      isAiVerified: (submission as any).isAiVerified || false,
      aiConfidence: (submission as any).aiConfidence || 0,
    });
    ...
    this.checkWatchlistAlerts(submission);

    await batch.commit();
    return submissionRef.id;
  },

  async checkWatchlistAlerts(submission: any) {
    try {
      const q = query(
        collection(db, 'watchlists'),
        where('itemName', '==', submission.itemName.toLowerCase())
      );
      const snap = await getDocs(q);
      
      snap.docs.forEach(async (d) => {
        const watchlist = d.data();
        if (watchlist.userId === submission.userId) return; // Don't notify self
        
        // Simple alert: if price is lower than user's target or just any new price
        if (!watchlist.targetPrice || submission.price <= watchlist.targetPrice) {
          // In a real app, we'd send a push notification here
          console.log(`Alert for user ${watchlist.userId}: ${submission.itemName} is now ${submission.price} EGP at ${submission.storeName}`);
        }
      });
    } catch (error) {
      console.error('checkWatchlistAlerts error:', error);
    }
  },

  async addToWatchlist(userId: string, itemName: string, targetPrice?: number) {
    const q = query(
      collection(db, 'watchlists'),
      where('userId', '==', userId),
      where('itemName', '==', itemName.toLowerCase())
    );
    const snap = await getDocs(q);
    if (!snap.empty) return; // Already watching
  ...
  async removeFromWatchlist(watchlistId: string) {
    // We'd need a deleteDoc here, but for simplicity let's assume we have the ID
  },

  getWatchlist(userId: string, callback: (items: any[]) => void) {
  ...
  async addToShoppingList(userId: string, itemName: string) {
  ...
  getShoppingList(userId: string, callback: (items: any[]) => void) {
  ...
  async calculateBestStoreForList(userId: string, items: string[], center: [number, number]) {
    // 1. Get nearby submissions for all items in the list
    const nearby = await this.getNearbySubmissions(center, 10000);
    
    // 2. Group by store
    const storeStats: { [storeName: string]: { total: number; count: number; items: string[] } } = {};
    
    items.forEach(item => {
      const lowerItem = item.toLowerCase();
      nearby.forEach(sub => {
        if (sub.itemName.toLowerCase().includes(lowerItem)) {
          if (!storeStats[sub.storeName]) {
            storeStats[sub.storeName] = { total: 0, count: 0, items: [] };
          }
          // Only take the cheapest price for this item at this store
          storeStats[sub.storeName].total += sub.price;
          storeStats[sub.storeName].count += 1;
          storeStats[sub.storeName].items.push(sub.itemName);
        }
      });
    });

    // 3. Sort by most items found, then by total price
