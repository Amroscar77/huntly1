...
export default function AIChatScreen() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: "Hello! I'm Huntly AI, your shopping assistant for Egypt. I can help you find the best deals, compare prices, and locate nearby stores. How can I help you today?" }
  ]);
  ...
  useEffect(() => {
    const checkKey = async () => {
      ...
      const envKey = (process.env as any)?.API_KEY || (process.env as any)?.GEMINI_API_KEY || (import.meta as any).env?.VITE_GEMINI_API_KEY;
      if (envKey) {
  ...
  if (!isKeySelected) {
    return (
      <div className="flex flex-col h-screen bg-surface">
        <header className="px-6 py-4 bg-surface-container border-b border-outline-variant/10 flex items-center gap-4">
          <button onClick={() => navigate(-1)} className="p-2 -ml-2 hover:bg-surface-container-high rounded-full transition-colors">
            <ChevronLeft size={24} />
          </button>
          <h1 className="font-headline text-xl font-black text-on-surface">Huntly AI</h1>
        </header>
        <div className="flex-grow flex flex-col items-center justify-center p-8 text-center">
          <div className="w-20 h-20 bg-primary/10 rounded-full flex items-center justify-center mb-6">
            <Bot size={40} className="text-primary" />
          </div>
          <h2 className="font-headline text-2xl font-black text-primary mb-4">Gemini API Key Required</h2>
          <p className="text-on-surface-variant text-sm mb-6 max-w-xs">
            To use the AI shopping assistant, please select your Gemini API key.
          </p>
          <p className="text-[10px] text-outline mb-8 max-w-xs uppercase tracking-widest">
            Note: You must select an API key from a paid Google Cloud project. 
            See <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noopener noreferrer" className="text-primary underline">billing docs</a>.
          </p>
          <button 
            onClick={handleSelectKey}
            className="signature-gradient text-white px-8 py-4 rounded-full font-bold shadow-xl active:scale-95 transition-all w-full max-w-xs"
          >
            Select API Key
          </button>
        </div>
