...
import { Settings, Award, History, ShieldCheck, MapPin, LogOut, Coins, Zap, Bell, Flame, PlusCircle } from 'lucide-react';
...
export default function ProfileScreen() {
  ...
  const [activity, setActivity] = useState<PriceSubmission[]>([]);
  ...
  useEffect(() => {
    if (!user) return;
    MarketService.getUserActivity(user.uid, 10).then((data) => {
      setActivity(data);
  ...
  const avatarUrl = user?.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(userData.displayName || 'U')}&background=ca5cdd&color=fff&size=120`;
  ...
  return (
    <div className="max-w-2xl mx-auto pb-20">
      ...
      <div className="px-6 -mt-12 relative z-10 grid grid-cols-4 gap-3">
        ...
        <StatCard icon={<ShieldCheck size={18} />} label="Trust" value={`${userData.trustScore || 100}%`} color="text-emerald-600" />
      </div>
      ...
      <div className="px-6 mt-10 space-y-8">
        ...
        <section>
          ...
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
            <AchievementBadge icon="🛒" label="First Post" color="bg-emerald-100" unlocked={userData.pricesAdded >= 1} />
            <AchievementBadge icon="🔥" label="7 Day Streak" color="bg-orange-100" unlocked={userData.maxStreak >= 7} />
            <AchievementBadge icon="💎" label="1000 Points" color="bg-purple-100" unlocked={userData.points >= 1000} />
            <AchievementBadge icon="🤝" label="Trusted" color="bg-blue-100" unlocked={userData.trustScore >= 90} />
            <AchievementBadge icon="📸" label="10 Posts" color="bg-pink-100" unlocked={userData.pricesAdded >= 10} />
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/10 p-6 shadow-lg flex items-center justify-between">
            ...
            >
              {notifLoading ? 'Enabling...' : notifEnabled ? 'Enabled ✓' : 'Enable'}
            </button>
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/10 overflow-hidden shadow-lg">
            {activityLoading ? (
              <div className="p-6 text-center text-outline">Loading activity...</div>
            ) : activity.length === 0 ? (
              <div className="p-12 text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-outline mb-4">Start contributing to earn points and climb the rankings!</p>
                  <Link 
                    to="/add" 
                    className="inline-flex items-center gap-2 bg-primary text-white px-6 py-2 rounded-full font-bold text-sm hover:scale-105 active:scale-95 transition-all shadow-md"
                  >
                    <PlusCircle size={18} />
                    Post First Price
                  </Link>
              ...
              activity.map((item) => (
                <Link
                  to={`/item/${item.id}`}
                  key={item.id}
                  className="flex items-center justify-between p-5 border-b border-outline-variant/5 last:border-0 hover:bg-surface-container-low transition-colors"
                >
                  <div>
                    <p className="font-bold text-on-surface">Posted: {item.itemName}</p>
                    <p className="text-xs text-outline font-medium">
                      {item.timestamp?.toDate ? formatDistanceToNow(item.timestamp.toDate(), { addSuffix: true }) : 'Recently'} · {item.storeName}
                    </p>
                  </div>
                  <div className="text-right">
                    <div className="bg-primary/10 text-primary px-3 py-1 rounded-full text-xs font-bold">+10 pts</div>
                    <p className="text-xs text-outline mt-1">{item.price} EGP</p>
                  </div>
                </Link>
              ))
            )}
...
function AchievementBadge({ icon, label, color, unlocked }: { icon: string; label: string; color: string; unlocked: boolean }) {
  return (
    <div className={cn('flex flex-col items-center gap-2 min-w-[80px] transition-opacity', !unlocked && 'opacity-30')}>
