...
export default function RankingsScreen() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 pt-8 pb-20 space-y-8">
      ...
      {/* List */}
      <div className="bg-surface-container-lowest rounded-3xl overflow-hidden border border-outline-variant/10 shadow-xl min-h-[400px] flex flex-col">
        {loading ? (
          <div className="flex-grow flex flex-col items-center justify-center p-12 space-y-4">
            <div className="w-12 h-12 border-4 border-primary border-t-transparent rounded-full animate-spin"></div>
            <p className="text-outline font-bold uppercase tracking-widest text-xs">Loading leaderboard...</p>
          </div>
        ) : users.length === 0 ? (
          <div className="flex-grow flex flex-col items-center justify-center p-12 text-center space-y-4">
            <Trophy size={48} className="text-outline/20" />
            <p className="text-outline font-bold">No leaders yet.</p>
            <p className="text-xs text-outline/60 uppercase tracking-widest">Be the first to post a price and lead the market!</p>
          </div>
        ) : (
