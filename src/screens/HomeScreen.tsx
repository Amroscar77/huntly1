  ...
  return (
    <div className="max-w-7xl mx-auto px-6 pt-8 space-y-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-gradient-to-br from-primary to-tertiary rounded-3xl p-8 text-white relative overflow-hidden shadow-xl shadow-primary/20">
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 h-full">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-4">
                <Sparkles size={20} className="text-primary-container" />
                <h2 className="font-headline text-3xl font-black tracking-tight">Huntly AI Insights</h2>
              </div>
              <p className="text-primary-container text-lg mb-6 opacity-90 leading-relaxed font-medium italic">"{aiAdvice}"</p>
      ...
      <div className={cn('relative overflow-hidden', isLarge ? 'flex-grow min-h-48' : 'h-48')}>
        ...
        {item.isAiVerified && (
          <div className="absolute top-4 left-4 z-10 flex flex-col gap-1">
            <span className="bg-primary/90 backdrop-blur-md text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1 shadow-lg">
              <Sparkles size={12} /> AI Verified
            </span>
            {item.aiConfidence && (
              <span className="bg-black/40 backdrop-blur-sm text-white/90 px-2 py-0.5 rounded-full text-[8px] font-bold uppercase tracking-widest self-start">
                {Math.round(item.aiConfidence * 100)}% Confidence
              </span>
            )}
          </div>
        )}
        {item.imageUrl && !item.isAiVerified && (
          <div className="absolute top-4 right-4 z-10">
            <span className="bg-success/90 backdrop-blur-md text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1 shadow-lg">
        ...
        {item.isSponsored && (
          <div className="absolute top-4 right-4 z-10">
            <span className="bg-warning text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg">Sponsored</span>
          </div>
        )}
        {item.verified && (
