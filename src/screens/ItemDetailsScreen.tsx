...
import { ChevronLeft, MapPin, Store, Clock, Verified, ThumbsUp, AlertTriangle, History, Lightbulb, Share2, Loader2, Sparkles, ShoppingCart, Plus, CheckCircle2 } from 'lucide-react';
...
export default function ItemDetailsScreen() {
  ...
  const handleAddToList = async () => {
    ...
    setListLoading(true);
    try {
      await MarketService.addToShoppingList(user.uid, item.itemName);
      setListSuccess(true);
      ...
      <div className="px-6 -mt-12 relative z-10 space-y-8">
              ...
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                ...
                {item.isAiVerified && (
                  <div className="flex flex-col gap-1">
                    <span className="bg-primary text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1 shadow-sm">
                      <Sparkles size={12} /> AI Verified
                    </span>
                    {item.aiConfidence && (
                      <span className="text-[9px] font-bold text-primary/60 uppercase tracking-widest ml-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <button 
            ...
            className={cn(
              "rounded-2xl p-6 flex items-center gap-4 transition-all active:scale-95 border",
              listSuccess 
          ...
          >
            <div className={cn(
              ...
              listSuccess ? "bg-emerald-100" : "bg-primary/10 text-primary"
            )}>
              {listLoading ? <Loader2 size={24} className="animate-spin" /> : listSuccess ? <CheckCircle2 size={24} /> : <ShoppingCart size={24} />}
            </div>
            <div className="text-left">
              <p className="text-xs font-black uppercase tracking-widest mb-1">{listSuccess ? 'Added!' : 'Shopping List'}</p>
              <p className="font-bold text-on-surface text-lg">{listSuccess ? 'Item in your list' : 'Add to smart list'}</p>
            </div>
          </button>
        </div>
